...
const showProduct = ({ id, title, image, price }) => {
...
const addToCart = (id) => {
  ...
  const itemFromProduct = products.find((product) => product.id === itemId);

  item
    ? (item.qty += 1)
    : cart.push({
        id: itemFromProduct.id,
        price: itemFromProduct.price,
        qty: 1,
      });

  totalItem.innerText = cart.reduce((acc, item) => acc + item.qty, 0);
};
